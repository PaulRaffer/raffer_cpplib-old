...
template <typename T> template <typename T1> constexpr color<T>::color(color<T1> const & rhs) : red(rhs.get_red()), green(rhs.get_green()), blue(rhs.get_blue()) {}
...
auto operator<<(std::ostream & os, color<auto> const & rhs) -> auto & { return os << "(" << rhs.get_red() << "," << rhs.get_green() << "," << rhs.get_blue() << ")"; }
